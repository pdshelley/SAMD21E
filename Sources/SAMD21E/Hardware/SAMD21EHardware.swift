...
public struct SAMD21EHardware {
  ...
  public init() {
    if !SAMD21EHardware.initialized {
      SAMD21EHardware.initialized = true
      resets.reset.set([
        .uart1,
        .uart0,
        .timer,
    ...
    func initializeClocks() {
      ...
      SAMD21EHardware.referenceClockHz = clocks.reference.configure(
        UInt32(xoscKHz) * UInt32(freq1KHz), UInt32(xoscKHz) * UInt32(freq1KHz))
      SAMD21EHardware.systemClockHz = clocks.system.configure(
...
extension SAMD21EHardware.Clocks.Reference {
...
extension SAMD21EHardware.Clocks.System {
  func configure(_ sourceFrequency: UInt32, _ frequency: UInt32) -> UInt32 {
    precondition(sourceFrequency >= frequency)
    let div = UInt32((UInt64(sourceFrequency) << 8) / UInt64(frequency))
    control.clearSource()
